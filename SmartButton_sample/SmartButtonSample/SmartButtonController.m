...
#define kTargetDeviceName          @"Smart Button"

#define kBatteryLevelServiceUUID   @"180F"
#define kBatteryLevelUUID          @"2A19"

#define kSwitchStatusServiceUUID   @"FFE0"
#define kSwitchStatusUUID          @"FFE1"

@interface SmartButtonController() <CBCentralManagerDelegate, CBPeripheralDelegate> {
    ...
    CBCharacteristic *_switchStatusCharacteristics;

    
    CBUUID *_batteryLevelServiceUUID;
    CBUUID *_batteryLevelUUID;

    CBUUID *_switchStatusServiceUUID;
    CBUUID *_switchStatusUUID;
}
...
@property (nonatomic) BOOL isSwitch1Pushed;
@property (nonatomic) BOOL isSwitch2Pushed;
@end

@implementation SmartButtonController
...
@synthesize isSwitch1Pushed;
@synthesize isSwitch2Pushed;
...
-(id)init {
    ...
    if(self) {
        ...
        _switchStatusServiceUUID     = [CBUUID UUIDWithString:kSwitchStatusServiceUUID];
        _switchStatusUUID  = [CBUUID UUIDWithString:kSwitchStatusUUID];
    }
    return self;
}
-(void)dealloc {
...
-(void)disconnectIntrinsic {
    ...
    _switchStatusCharacteristics  = nil;
        
    self.isSwitch1Pushed = NO;
    self.isSwitch2Pushed = NO;
    self.deviceRSSI     = 0;
    self.batteryLevel   = 0;
...
-(void)startScanning {
    ...
    NSArray *scanServices = [NSArray arrayWithObjects:_switchStatusServiceUUID, _batteryLevelServiceUUID, nil];
    ...
    for (CBService *service in peripheral.services) {
        if ([service.UUID.data isEqualToData:_batteryLevelServiceUUID.data]) {
            [peripheral discoverCharacteristics:[NSArray arrayWithObjects:
                                                 ...
                                                 nil]
                                     forService:service];
        } else if ([service.UUID.data isEqualToData:_switchStatusServiceUUID.data]) {
            [peripheral discoverCharacteristics:[NSArray arrayWithObjects:
                                                 _switchStatusUUID,
                                                 nil]
                                     forService:service];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error
{
    if ([service.UUID.data isEqualToData:_batteryLevelServiceUUID.data]) {
        ...
        _switchStatusCharacteristics = [self findCharacteristics:service.characteristics uuid:_switchStatusUUID];
        [peripheral readValueForCharacteristic:_switchStatusCharacteristics];
        [peripheral setNotifyValue:YES forCharacteristic:_switchStatusCharacteristics];
    }
}
/*
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if(characteristic == _batteryLevelCharacteristics) {
        [characteristic.value getBytes:&b length:1];
        self.batteryLevel = b;
    } else if(characteristic == _switchStatusCharacteristics) {
        [characteristic.value getBytes:&b length:1];
        self.isSwitch1Pushed = ((b & 0x02) != 0);
        self.isSwitch2Pushed = ((b & 0x04) != 0);
NSLog(@"sw1: %d sw2: %d ", self.isSwitch1Pushed, self.isSwitch2Pushed);
    }
}
@end
